var knightpath=function(){};
// internal variables
knightpath.prototype.sizeX=450;//preferred width of the game board in px
knightpath.prototype.sizeY=450;//preferred height of the game board in px
knightpath.prototype.rankX=8;//rank of the board on the x-axis
knightpath.prototype.rankY=8;//rank of the board on the y-axis
knightpath.prototype.fieldArr=[];//the board itself
knightpath.prototype.curId=0;//number of the last move made
knightpath.prototype.allowedSquares=0;//number of squares allowed to move into after last move
knightpath.prototype.lastPos=[];//coordinates of last move
knightpath.prototype.wantsAutoMove=true;//use auto move?
knightpath.prototype.figureMoves=[2,1];//movement shape of the piece used (knight - (2,1), cammel - (3,1), zebra - (3,2))
// interface
knightpath.prototype.gameBoardId="game-board";//id of div designated to be gameboard
knightpath.prototype.gameBoardRowClass="row";//css class for gameboard rows
knightpath.prototype.gameBoardColumnClass="col";//css class for gameboard rows
// square styles interfaces
knightpath.prototype.squareClass="cell";//css class for all squares
knightpath.prototype.allowedSquareClass="allowed";//css class for allowed squares
knightpath.prototype.autoMoveSquareClass="automoved";//css class for automoved
knightpath.prototype.currentSquareClass="current";//css class for current square
// messaging interface
knightpath.prototype.messageInterfaceId="log";//id of div containing messages
knightpath.prototype.clearMessageInterface=function() {$("#"+this.messageInterfaceId).text("").css("display","none");}
knightpath.prototype.prepareMessageInterface=function() {$("#"+this.messageInterfaceId).css("display","block");}
knightpath.prototype.appendToMessageInterface=function(t) {$("#"+this.messageInterfaceId).append("<p>"+t+"</p>");}
// functions
knightpath.prototype.initialize=function () {
...
knightpath.prototype.fontFraction=function() {
	var full=this.rankX*this.rankY;
	if(full<10) return 0.75;
	else if(full<100) return 0.6;
	else return 0.45;
}
knightpath.prototype.isAllowed=function(x,y) {return this.isSquareVal(x,y,0);}
knightpath.prototype.allowSquare=function(x,y) {
	this.setSquareValue(x,y,0);
	this.allowedSquares++;
}
knightpath.prototype.isEmpty=function(x,y) {return this.isSquareVal(x,y,-1);}
knightpath.prototype.emptySquare=function(x,y) {this.setSquareValue(x,y,-1);this.setSquareText(x,y,"");}
knightpath.prototype.isSquareVal=function(x,y,v) {return (this.fieldArr[y][x]==v);}
knightpath.prototype.setSquareValue=function(x,y,v) {this.fieldArr[y][x]=v;}
knightpath.prototype.makeMoveTo=function(x,y) {
	this.curId++;
	this.lastPos=[x,y];
...
knightpath.prototype.undo=function() {
	this.clearMessageInterface();
	if(this.curId==1) {
		this.initialize();
		return ;
...
knightpath.prototype.obtainMovePattern=function(x,y) {
	var rez=[];
...
knightpath.prototype.obtainSingleMove=function(x,y,d) {
...
knightpath.prototype.applyMovePattern=function(x,y) {
	// forbid all possible
	for(var yi=0;yi<this.rankY;yi++) {
		for(var xi=0;xi<this.rankX;xi++) {
...
knightpath.prototype.autoMove=function() {
...
knightpath.prototype.addSquareClass=function(x,y,c) {
	$(this.getSquareId(x,y)).addClass(c);
}
knightpath.prototype.removeClass=function(c) {
	$("#"+this.gameBoardId+" ."+c).removeClass(c);
}
knightpath.prototype.setSquareText=function(x,y,t) {
	$(this.getSquareId(x,y)).text(t);
}
knightpath.prototype.getSquareId=function(x,y) {
	return "#c"+y+"x"+x;
}
knightpath.prototype.onSquareClick=function(id) {
...
knightpath.prototype.highLightSquares=function() {
...
knightpath.prototype.isClosedTour=function() {
...
knightpath.prototype.gameOver=function() {
